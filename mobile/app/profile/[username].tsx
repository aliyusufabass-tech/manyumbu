...
export default function OtherProfileScreen() {
  ...
  return (
    <ScrollView contentContainerStyle={{ padding: 20, paddingTop: 56, gap: 16 }} style={{ backgroundColor: "#FFFFFF" }}>
      ...
      <Text style={{ fontWeight: "800", fontSize: 18 }}>Posts</Text>{profile.viewer_can_view_private_content ? <PostGrid username={profile.username} /> : <Text style={{ color: "#52605B" }}>Follow this private account to see posts.</Text>}<Text style={{ fontWeight: "800", fontSize: 18 }}>Reels</Text>{profile.viewer_can_view_private_content ? <ReelGrid username={profile.username} /> : null}
    </ScrollView>
  );
}
