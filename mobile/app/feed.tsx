...
export default function FeedScreen() {
  ...
  return <View style={{ flex: 1, backgroundColor: "#FFFFFF", paddingTop: 48, paddingHorizontal: 18 }}><View style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "center" }}><Text style={{ fontSize: 26, fontWeight: "800" }}>Home</Text><Pressable onPress={() => router.push("/posts/create")}><Text style={{ color: "#126C57", fontWeight: "800" }}>Create</Text></Pressable></View><FlatList data={query.data.data.results} keyExtractor={(item) => item.id} refreshing={query.isFetching} onRefresh={() => query.refetch()} ListEmptyComponent={<ScreenState text="Follow people or create a post to start your feed." />} renderItem={({ item }) => <PostCard post={item} onOpen={() => router.push({ pathname: "/posts/[id]", params: { id: item.id } })} onLike={() => like.mutate(item)} onSave={() => save.mutate(item)} />} /></View>;
}
